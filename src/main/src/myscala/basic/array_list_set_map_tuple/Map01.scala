...
object Map01 {
  def main(args: Array[String]): Unit = {
    /**
      ...
      */
    val map = Map[String, Int]("c" -> 100, "b" -> 200, ("a", 300), ("a", 400)) // Map中元素是 元组Tuple2
    println(map)
    map.foreach(println)
    ...
    //val value: Int = map.get("aa").get // map("aa") 直接拿到值，不是Option。 NoSuchElementException: None.get
    //println(value)
    // map.get("aa").getOrElse(0) 等价于
    val i: Int = map.getOrElse("aa", 0) // map.getOrElse("aa", 0)
    println(i)
